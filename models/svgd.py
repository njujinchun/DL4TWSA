...
class SVGD(object):
    ...
    def train(self, epoch, logger):
        ...
        self.bayes_nn.train()
        mse = 0.
        for batch_idx, (input, hist, target) in enumerate(self.train_loader):
            ...
            for i in range(self.n_samples):
                output_i = self.bayes_nn[i].forward(input,hist)
        ...
        print("epoch {}, training r2: {:.4f}".format(epoch, r2_train))

        for i in range(self.n_samples):
            self.schedulers[i].step(rmse_train)
        print("lr: {}".format(self.optimizers[0].param_groups[0]['lr']))
